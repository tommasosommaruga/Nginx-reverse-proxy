...
log_pattern = re.compile(
    ...
    r'(?P<request_time>[\d.]+) (?P<upstream_time>[\d.]+|-) (?P<host>\S+)'
)
...
def parse_log_line(line):
    ...
    data["upstream_time"] = float(data["upstream_time"]) if data["upstream_time"] != "-" else 0.0
    return data
...
def update_graphs(n):
    ...
    fig5 = px.box(df, y="request_time", title="Request Time Distribution")
    fig6 = px.box(df, y="upstream_time", title="Upstream Response Time Distribution")
