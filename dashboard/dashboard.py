...
# bot_keywords = [
#     'bot', 'crawl', 'spider', 'scanner', 'monitor', 'fetch', 'wget', 'curl',
#     'python-requests', 'httpclient', 'libwww', 'Go-http-client', 'l9scan'
# ]
...
def update(n, start_date, end_date, ip_contains, url_contains, ua_contains, methods, statuses, countries, bot_filter,bin_size):
    df = load_data()
    if df.empty:
    ...
    if df.empty:
        return *[{'layout': {'title': 'No matching data'}}]*5, [], [], [], "Total Requests: 0"
    ...
    ip_table = df.groupby(['ip', 'country', 'latitude', 'longitude', 'user_agent', 'is_bot', 'isp', 'usage_type']).size().reset_index(name='count')
    ...
    return world_fig, time_fig, status_fig, top_ip_fig, top_path_fig, table_data, tooltip_data, method_opts, country_opts, status_opts, f"Total Requests: {len(df)}"
